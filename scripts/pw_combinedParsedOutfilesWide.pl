...
my $outfile = "allAlignments.PAMLsummaries.wide.txt";
my $overwrite = 0;
...
foreach my $file (@ARGV) {
    ...
    if ($file !~ m/PAMLsummary\.wide.txt/) {
        die "\n\nTerminating - did you really want to run this script on file $file ? It is not a PAMLsummary.wide.txt file. If you did mean that, change the script\n\n";
    }
    open (IN, "< $file");
    while (<IN>) {
        my $line = $_;
