...
```
docker tag paml_wrapper jayoungfhcrc/paml_wrapper:version1.0.8
docker push jayoungfhcrc/paml_wrapper:version1.0.8
```
...
```
docker run -it jayoungfhcrc/paml_wrapper:version1.0.8
...
singularity build paml_wrapper-v1.0.8.sif docker://jayoungfhcrc/paml_wrapper:version1.0.8
module purge
```
A file called paml_wrapper-v1.0.8.sif appears. I want a copy of the singularity image file, and a script that uses it, in a more central place, for use by others:
```
cp paml_wrapper-v1.0.8.sif /fh/fast/malik_h/grp/malik_lab_shared/singularityImages
```
...
module load Singularity/3.5.3
singularity shell --cleanenv /fh/fast/malik_h/grp/malik_lab_shared/singularityImages/paml_wrapper-v1.0.8.sif
module purge
```
...
```
module load Singularity/3.5.3
singularity exec --cleanenv /fh/fast/malik_h/grp/malik_lab_shared/singularityImages/paml_wrapper-v1.0.8.sif pw_makeTreeAndRunPAML.pl CENPA_primates_aln2a_NT.fa 
module purge
```
